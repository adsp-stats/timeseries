...
$$\begin{aligned} \textrm{State}: \quad \boldsymbol{x}_t &= \overbrace{\mathbf{A} \boldsymbol{x}_{t-1}}^\textrm{Evolve} + \overbrace{\mathbf{B} \boldsymbol{u}_t}^\textrm{Force} + \boldsymbol{\omega}_t\\ \\ \textrm{Observation}: \quad \boldsymbol{y}_t &= \underbrace{\mathbf{F} \boldsymbol{x}_t}_\textrm{Measure} + \boldsymbol{\nu}_t \end{aligned}$$
 ...
 * $\mathbf{F} = [1]$ (nothing about state is hidden)
