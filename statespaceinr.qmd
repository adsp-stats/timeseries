...
```{r}
y    <- 100*stocks$CAT #scaling for stability
xreg <- 100*stocks$SP500RET #scaling for stability
...
cat.fit2 <- fitSSM(cat.mod, inits=init_pars, updatefn=update_fn, method="BFGS")
```
...
plot(x=as.Date(rownames(stocks)),y=state.ests.smooth[,2], type = "l",
     main = "State space model compared to roling regression",
